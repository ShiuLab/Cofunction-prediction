...
def combine(m1, m2, f1, f2, debug=0):
    ...
    if minV == 0:
        minV = pseudo
    v_m1_m2 = [m1 + m2, maxV - minV, m1 * m2, maxV / minV,
               minV, maxV, statistics.mean([m1, m2])]
    # feature name
...
while inl != "":
    try:
        [genepair, label, fit1, fit2, fit12, ADD, PRODUCT, LOG,
         MIN] = inl.strip().split(
            ',')  # a list of values ### 01/20/22 change #03/03/22 addtional change
    except ValueError:
    ...
    try:
        m_fit1 = transform_each(float(fit1), "",
                                debug)  # 0.9,0.9^2,sqrt(0.9),1/0.9
    ...
    if out_header == 0:
        oup.write(
            "Pair\tlabel\tFit1\tFit2\tFit12\tADD\tPRODUCT\tLOG\tMIN\t%s\n" % '\t'.join(
                f_gis2))
        out_header = 1

    # Write all values
    oup.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %
              (genepair, label, fit1, fit2, fit12, ADD, PRODUCT, LOG, MIN,
               "\t".join(m_gis2)))
